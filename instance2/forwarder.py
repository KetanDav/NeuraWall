#!/usr/bin/env python3
"""
Inline NGFW engine between tap2 <-> tap3.

Responsibilities:
- Capture packets from tap2 and tap3 using AF_PACKET raw sockets
...
CLASSIFIER_PLAIN_URL = "http://127.0.0.1:6000/fastclassify-plain"
CLASSIFIER_ENC_URL   = "http://127.0.0.1:6000/fastclassify-encrypted"
CLASSIFIER_L4_URL    = "http://127.0.0.1:6000/fastclassify-normal"
...
def main():
    ...
    print("[+] Inline forwarder (tap2<->tap3) with 10-packet warmup + ML classifier running (Ctrl+C to stop)")
    ...
    try:
        while True:
            ...
            if s0 in r:
                ...
                if addr[2] != PKT_OUT:
                    process_packet("tap2", "tap3", data, s1)
                else:
                    print("[DEBUG] Ignoring outgoing packet on tap2")
            if s1 in r:
                data, addr = s1.recvfrom(65535)
                if addr[2] != PKT_OUT:
                    process_packet("tap3", "tap2", data, s0)
                else:
                    print("[DEBUG] Ignoring outgoing packet on tap3")
